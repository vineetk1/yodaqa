 ...
 *
 * LatByHeadword generates a LAT for a Question using one of the following three methods: (1) Generate LAT if question has wh-word; (2) Generate LAT if question matches a pattern; (3) Generate LAT after finding the headword of the question.
 * Difference between LatByHeadword and FocusGenerator plus LatByFocus:
 * (1) LatByHeadword does not generate LATs of those wh-words that are generated by LatByFocus
 * (2) LatByHeadword generates LATs from Patterns that FocusGenerator+LatByFocus do not
 * (3) LatByHeadword and FocusGenerator use different algorithms to generate headword/focus; 
 *     Note that Headword and Focus are the same
 * (4) LatByHeadword finds the meaning of headword, whereas FocusGenerator does not
 *
 * Generate Headword by implementing the algorithms in "Silva, J. P. C. G. (2009). QA+ML@ Wikipedia&Google 
 ...
 *
 * Headword and Focus are the same. So, this implementation and FocusGenerator.java are both generating
 * the headword/focus. The difference is in the algorithm used, plus that this implementation also finds
 * the meaning of the headword/focus from the context of the Question with the use of WordNet. Also, this implementation and LatByFocus.java
 * are both generating LATs. This implementation does not generate LATs for those wh-words that are 
 * covered by LatByFocus.java.
 *
 * The LatByHeadword class traverses the parse tree of the question to find the headword. 
 * Major methods are as follows:
 * (1) questionMatchesPattern() filters out those questions that match a Pattern; these questions
 *     do not have a headword, but one can be inferred from the pattern
...
public class LatByHeadword extends JCasAnnotator_ImplBase {
	final Logger logger = LoggerFactory.getLogger(LatByHeadword.class);
...
//find headword, etc. ONLY if focus.class is absent
		for (ROOT rootNode : JCasUtil.select(jcas, ROOT.class)) {
//dumpCTree(rootNode, 0);
			if (!LatByWhWord(jcas, rootNode)) 
				if (!LatByPattern(jcas, rootNode)) {
					Token headNode = findRuleBasedHeadword(jcas, rootNode); 
				/* 
				if (headNode) 
					find Sense of headword
					write to LAT
					*/
			}  	
			else return;
	...
	private boolean LatByWhWord(JCas jcas, Constituent rootNode) {
		/* (1) Question text of certain wh-words do not have a headword. The headword is inferred 
		 * from wh-words as follows: when => time, date; where => location, organization; 
		 * who/whom => person; why => reason.
		 * (2) how => description, except when the word following "how" is a headword if it is
		 * an adjective or an adverb
		 *
		 * Wh-words are used as follows:
		 * LatByWhWord(): whom, why, what, which
		 * LatByPattern(): what, which
		 * FocusGenerator.java: when, where, who, how 
		 *
		 * (3) Question with 1 or 2 words => meaning/definition
		 */ 

		boolean latByWhWord = false;

/* only first token in the Question is needed for wh-word; so replace for-statement below with a 
 * statement that extracts first token only
 */
		for (Token token : JCasUtil.selectCovered(Token.class, rootNode)) {
			String qWord = token.getCoveredText().toLowerCase();

			if (qWord.startsWith("wh")) {
				switch (qWord) {
					case "when": case "where": case "who":
						logger.debug("No headword for wh-word: {}", token.getCoveredText()); 
						latByWhWord = true;
						break;
					case "whom":
...
// Question with 1 or 2 words => meaning/definition
	private boolean LatByPattern(JCas jcas, Constituent rootNode) {
		boolean questionMatchesPattern = true;
...
/*
			case "what": case "which": 
						 ...
						 * implemented; they should be implemented here
						 * 
						return !whWordFound;
*/
		}
		if (numberOfTokens <= 2) {
//actually LAT is Definition/Meaning/Description with pos=adjective??
			logger.debug("No headword, question has less than or equal to two tokens: {}",
					((Constituent)rootNode).getCoveredText());			
System.out.println("# questionMatchesPattern: no headword, question is less than equal to two tokens");
			return questionMatchesPattern; 
		}
	...
	private Token findRuleBasedHeadword(JCas jcas, Constituent node) {
